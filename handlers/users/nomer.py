...
from utils.db_api.create_user import async_session, User, Transaction, Numbers_list, Order_numbers
...
async def nomer_detail_handler(call: CallbackQuery):
    ...
    async with async_session() as session:
        result = await session.execute(
            select(Numbers_list).filter(Numbers_list.country == country_name)
        )
...
async def buy_number_handler(call: CallbackQuery):
    ...
    try:
        ...
        async with async_session() as session:
            result = await session.execute(select(User).filter(User.id == tg_id))
...
async def check_num(call: CallbackQuery):
    ...
    async with async_session() as session:
        result = await session.execute(
            select(Order_numbers).filter(Order_numbers.id == number_id)
        )
